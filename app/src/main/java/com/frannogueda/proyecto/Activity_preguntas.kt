    ...
    fun actualizarRanking (listaAux: ArrayList<JSONObject>): Boolean {
        ...
        for (usuario in listaAux) {
            if ((!salir) && (porcentajeActual >= usuario.getDouble("porcentaje"))) {
                salir = true // para dejar de recorrer el array
        ...
        vJSON = JSONArray(listaAux)// vuelvo a convertir el arreglo a JSONArray
        return aux
    }
    fun puntajeFinal() {
        ...
        agregarJugador() // modifica como quedo el jugar al final
        dialogo()
    }
